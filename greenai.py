...
if not TELEGRAM_TOKEN or not OPENAI_API_KEY or not WEBHOOK_URL:
    raise ValueError("❌ TELEGRAM_TOKEN, OPENAI_API_KEY yoki WEBHOOK_URL topilmadi! (Environment Variables'ni tekshiring)")
...
def load_catalog(force=False):
    ...
    try:
        ...
        # max_rows=150 olib tashlandi, katalog to'liq yuklanishi uchun
        catalog_data = df.to_string(index=False)
...
async def cmd_start(message: types.Message):
    ...
    if user_id not in stats["users"]:
        ...
        save_stats(stats)
        print(f"✅ Yangi foydalanuvchi (/start): {user_id} | Jami: {stats['total_users']}")
...
async def handle_text(message: types.Message):
    ...
    if user_id not in stats["users"]:
        ...
        stats["total_users"] = len(stats["users"])
        save_stats(stats)
        print(f"✅ Yangi foydalanuvchi (xabar): {user_id} | Jami: {stats['total_users']}")
...
async def main():
    ...
    # Render avtomatik beradigan PORTni ishlatish (yoki 10000)
